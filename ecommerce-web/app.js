...
// Define api routes here so as to not be affected by csurf
const apiRoutes = require('./routes/api/index');
app.use('/api', apiRoutes);

// Initialize csurf for csrf tokens
app.use(csurf());
...
app.use(function (err, req, res, next) {
  ...
  res.render('errors/403', {title: '403 | FORBIDDEN | VENDO', isAuthenticated: req.isAuthenticated()})
});

// app.use((err, req, res, next) => {
//   console.error(err.stack);
//   res.status(500).render('errors/500', {title: '500 | SERVER ERROR | VENDO', isAuthenticated: req.isAuthenticated()});
// });
